...
#   create = var.enable_keycloak

#   # Disable helm release
#   create_release = var.create_kubernetes_resources

#   # https://github.com/argoproj/argo-helm/blob/main/charts/argo-cd/Chart.yaml
#   name             = try(var.keycloak.name, "argo-cd")
#   description      = try(var.keycloak.description, "A Helm chart to install the ArgoCD")
#   namespace        = try(var.keycloak.namespace, "argocd")
#   create_namespace = try(var.keycloak.create_namespace, true)
#   chart            = try(var.keycloak.chart, "argo-cd")
#   chart_version    = try(var.keycloak.chart_version, "5.55.0") # TODO - v6.x
#   repository       = try(var.keycloak.repository, "https://argoproj.github.io/argo-helm")
#   values           = try(var.keycloak.values, [])

#   timeout                    = try(var.keycloak.timeout, null)
#   repository_key_file        = try(var.keycloak.repository_key_file, null)
#   repository_cert_file       = try(var.keycloak.repository_cert_file, null)
#   repository_ca_file         = try(var.keycloak.repository_ca_file, null)
#   repository_username        = try(var.keycloak.repository_username, null)
#   repository_password        = try(var.argocd.repository_password, null)
#   devel                      = try(var.keycloak.devel, null)
#   verify                     = try(var.keycloak.verify, null)
#   keyring                    = try(var.keycloak.keyring, null)
#   disable_webhooks           = try(var.keycloak.disable_webhooks, null)
#   reuse_values               = try(var.keycloak.reuse_values, null)
#   reset_values               = try(var.keycloak.reset_values, null)
#   force_update               = try(var.keycloak.force_update, null)
#   recreate_pods              = try(var.keycloak.recreate_pods, null)
#   cleanup_on_fail            = try(var.keycloak.cleanup_on_fail, null)
#   max_history                = try(var.keycloak.max_history, null)
#   atomic                     = try(var.keycloak.atomic, null)
#   skip_crds                  = try(var.keycloak.skip_crds, null)
#   render_subchart_notes      = try(var.keycloak.render_subchart_notes, null)
#   disable_openapi_validation = try(var.keycloak.disable_openapi_validation, null)
#   wait                       = try(var.keycloak.wait, false)
#   wait_for_jobs              = try(var.keycloak.wait_for_jobs, null)
#   dependency_update          = try(var.keycloak.dependency_update, null)
#   replace                    = try(var.keycloak.replace, null)
#   lint                       = try(var.keycloak.lint, null)

#   postrender    = try(var.keycloak.postrender, [])
#   set           = try(var.keycloak.set, [])
#   set_sensitive = try(var.keycloak.set_sensitive, [])
